	...
	allNodesAndMatchingSnippets.forEach(({node, snippet, matchingTerm}) => {
		...
		function mouseEnterHandler(event) {
			...
			sendMessageToBackgroundScript({
				subject: 'track',
				trackingProperties: ['browser_addon_tooltip_hovered', {title}, window.location.hostname]
			});
			...
			const handleLinkClick = () => sendMessageToBackgroundScript({
				subject: 'track',
				trackingProperties: ['browser_addon_tooltip_link_clicked', {title, href}, window.location.hostname]
			});
		...
		function textNodeReplace(node, regex, handler) {
			const mom = node.parentNode; const nxt = node.nextSibling;
			const doc = node.ownerDocument; let
				hits;
			if (regex.global) {
